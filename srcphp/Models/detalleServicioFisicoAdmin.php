...
class detalleServicioFisicoAdmin extends Models
{
    protected $fillable = [
    ...
    public function data($filters)
    {
        try {
            ...
            $sql = "
                 SELECT o_f.fecha, o_f.producto, o_f.problema,o_f.id_orden_fisica,
                ...
                WHERE o_f.fecha BETWEEN :start_date AND CURDATE()
            ";

            if (!empty($date)) {
                $sql .= " AND o_f.fecha >= :start_date";
            }
            if (!empty($clientName)) {
